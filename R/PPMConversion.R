...
PPMConversion <- function(RawSpect_data, RawSpect_info, method = c("thres", "max"), 
                          ...
                          c = 2, freq = TRUE, fromto.TMSP = NULL) {
  ...
  findTMSPpeak <- function(ft, c = 2) {
    ...
    if (is.na(v))  {
      ...
      # recherche dans les 1% de points suivants du max trouve pour etre au sommet du pic
  ...
  if (is.null(fromto.TMSP)) {
    ...
    for (i in 1:length(fromto.TMSP)) {
      Int[[i]] <- indexInterval(colindex, from = fromto.TMSP[[i]][1], to = fromto.TMSP[[i]][2], 
                                inclusive = TRUE)
    }
  ...
  if (shiftHandling == "zerofilling" || shiftHandling == "NAfilling") {
    ...
    Spectrum_data <- matrix(fill, nrow = n, ncol = end - start + 1, dimnames = 
    ...
    for (i in 1:n)  {
      Spectrum_data[i, ] <- RawSpect_data[i, (1 + (TMSPpeaks[i] - minpeak)):(m - 
        (maxpeak - TMSPpeaks[i]))]
    }
  } else {
    # circular
