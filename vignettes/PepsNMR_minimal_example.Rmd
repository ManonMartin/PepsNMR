...
require(knitr)
knitr::opts_chunk$set(collapse = TRUE, comment = "#>",  fig.width = 10, fig.height = 4, fig.align = "center", out.width = "600px", tidy = TRUE,
...
You can install the library either directly from Github [https://github.com/ManonMartin/PepsNMR.git](https://github.com/ManonMartin/PepsNMR.git)
:
```{r install1,  eval = FALSE, background = "#EFFBF5", highlight = TRUE}

# install.packages("devtools") # need to be installed at first 
require ("devtools")
devtools::install_github("ManonMartin/PepsNMR", dependencies = TRUE)
require(PepsNMR)

```

Or download a standalone version from Github to a local folder and install from it: 
...
To import FIDs in Bruker format, use the `ReadFids` function. This function will return a list with the FID data matrix (here saved as `Fid_data`) and information about these FIDs (here saved as `Fid_info`).
...
All the pre-treatment steps on FIDs and spectra can be easily applied to the data. Here is the recommanded pre-treatment workflow once data have been loaded:

```{r steps, echo=FALSE,  eval=TRUE}

Steps <- c("First Order Phase Correction", 
           "Solvent Suppression",
           "Apodization",
           "Fourier Transform",
...
StepsDescr <- c("Correct for the Bruker Group Delay.",
                ...
                "Proceed to data reduction.",
                "Set a desired region to 0.",
...
Information for each function is available, (e.g. type `?ReadFids` in the R console).
...
## Load the data
This dataset is already present in the PepsNMR package. We can load the FIDs from where the PepsNMR package is intalled:
```{r}
...
The first step is meant to access the Bruker data files.
Possible folders structure can be seen in Figure X
...
<img src="ReadFids.png" alt="Drawing" style="width: 400px;"/>
...
Note: the function `ZoneAggregation` is not used here, but is can be applied to urine spectra to aggregate the citrate peak.

# Final spectra visualisation 

The `Draw` function enables to produce line plots `type.draw = "signal"` or PCA `type.draw = "pca"` of FIDs or spectra. These plots can be saved as pdf files with the option `output = "pdf"`, see `?Draw` for more details on available options.

```{r visu1, fig.height=7, fig.width=7, error=FALSE}
Draw(Spectrum_data.N[1:4,], type.draw = c("signal"), subtype= "stacked", output = c("default"))
```


```{r visu2, fig.height=6, fig.width=6, out.width='70%', eval=TRUE, warning=FALSE}
data("Group_HS")
